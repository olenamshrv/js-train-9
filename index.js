...
function findElementGreaterThan(arr, num) {
  ...
  let numExists = arr.find((element) => element > num);
  if (numExists === undefined) {
...
function findLastEvenNumber(arr) {
  ...
  let last = arr.findLast((elem) => elem % 2 === 0);
  return last === undefined ? "Парних чисел не знайдено" : last;
}
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let sliceArray = arr.slice(arr.length - n);
  sliceArray.forEach((element) => console.log(element));

  return sliceArray.reduceRight(
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((element) => Array.from(element).length);
  return lengthsArray;
}
...
function expandArrayByN(arr, n) {
  ...
  if (!Array.isArray(arr) || !typeof n) {
    return "Перший вхідний параметр має бути масивом, другий - числом";
  }
  return arr.flatMap((element, index, flattedArray) => {
    let elemArray = [element];
    for (let i = 1; i < n; i++) {
      elemArray.splice(i, 0, element);
...
function findLongestWord(arr) {
  ...
  return arr.reduce((longestWord, currentWord) => {
    if (Array.from(currentWord).length > Array.from(longestWord).length) {
      return currentWord;
...
function findDuplicateElements(arr) {
  ...
  return arr.filter((element, index, elemArray) => {
...
function capitalizeWords(arr) {
  ...
  return arr.map((element) => {
    let word = Array.from(element.toString());
    element = [word[0].toUpperCase()].concat(word.slice(1)).join("");
    return element;
  });
}
...
function calculateTotalPrice(arr) {
  ...
  return arr.reduce(
    (totalPrice, currentElement) => totalPrice + currentElement.price,
    0
  );
}
